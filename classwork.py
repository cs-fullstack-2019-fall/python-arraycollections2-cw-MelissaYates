# Python review CW
# Problem 1:
# Create code and define the variable below outside of any function. After you create the list variable write a function called
# ```stupid_array_tricks``` that takes an array as a parameter, and performs the functions listed below in the instructions.
# ```
...
def stupid_array_tricks():
    for x in range(1, 3):
        print(person_array[x])
    # !! : Print the last 2 elements of the person_array using a *range*
    # !! : Insert the new element ```Chuck``` between the 2nd (```Kevin```) and 3rd (```Erin```) elements
    # !! : Take out the 2nd element (```Kevin```) from the list
...
def data():
    print(f'Starting length of data list is {len(data_list)}')
    for eachElement in data_list:
#TODO: come back and refactor. code is klugey NICE COMMENT
        if eachElement =='BAD_DATA':
            data_list.remove('BAD_DATA')
            print(data_list) # !! : no need to print in each iteration
            print(f'Ending length of data list is {len(data_list)}') # !! : just move this line out of your loop and you're golden!
...
score_list = [21,14,6,8,28,42,21]
def math_probs():
    print(sum(score_list)) # !! : use some string formatting for readability in the console 
    print(max(score_list)) # !! : use some string formatting for readability in the console
    print(min(score_list)) # !! : use some string formatting for readability in the console
